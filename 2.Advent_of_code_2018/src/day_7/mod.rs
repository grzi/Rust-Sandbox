...
fn find_order(_correspondances_vec : &Vec<(char,char)>) -> String{
    ...
    while graph.len() != 0 {
        ...
        graph.remove(&tmp);
        graph.values_mut().for_each(|value| {value.remove(&tmp).to_string();()});
    }
    result
}
...
fn find_time_with_workers(_correspondances_vec : &Vec<(char,char)>) -> u32{
    ...
    while graph.len() != 0 {
        ...
        graph.remove(&tmp.1);
        graph.values_mut().for_each(|value| {value.remove(&tmp.1).to_string();()});
    }
...
mod tests {
    ...
    #[test]
    fn test1() {
        assert_eq!(read_input(&"Step C must be finished before step A can begin."), ("C".to_string(),"A".to_string()));
    }
}
